...
mod tests {
    ...
    #[test]
    fn should_be_able_to_get_calendar_weeks_with_start_as_sunday_in_lua() {
        macro_rules! test {
            (($year:expr, $month:expr, $day:expr), $expected:expr) => {{
                let date = PdfDate(NaiveDate::from_ymd_opt($year, $month, $day).unwrap());
                let value = Lua::new()
                    .load(chunk!($date:calendar_week_sunday()))
                    .eval::<u8>()
                    .unwrap();
    ...
    #[test]
    fn should_be_able_to_get_calendar_weeks_with_start_as_monday_in_lua() {
        macro_rules! test {
            (($year:expr, $month:expr, $day:expr), $expected:expr) => {{
                let date = PdfDate(NaiveDate::from_ymd_opt($year, $month, $day).unwrap());
                let value = Lua::new()
                    .load(chunk!($date:calendar_week_monday()))
                    .eval::<u8>()
                    .unwrap();
                assert_eq!(value, $expected);
            }};
