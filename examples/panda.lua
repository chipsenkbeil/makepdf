...
pdf.hooks.on_weekly_page(function(page)
    ...
    page.push(pdf.object.group({
        make_single_line({
            bounds = grid.cell({ row = 1, col = 2, width = 2 }):with_padding(SPACING),
            text = page.date:format("%B %d")
                .. " to "
                -- TODO: This does not work for end of year as it wraps around
                .. page.date:end_of_week_monday():format("%d"),
        }),
