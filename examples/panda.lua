...
pdf.hooks.on_weekly_page(function(page)
    ...
    page.push(make_single_line({
        bounds = grid.cell({ row = 1, col = 2, width = 2 }):with_padding(SPACING),
        text = page.date:format("%B %Y (" .. tostring(page.date.week) .. ")"),
    }))
    ...
    page.push(make_section({
        bounds = grid.cell({ row = 10.25, col = 1, width = 7, height = 9 }),
        ...
        on_inner = function(opts)
            ...
            table.insert(group, pdf.object.text({ text = "PERSONAL" })
                :align_to(g.cell({ row = 1, col = 1 }), { v = "top", h = "left" }))
            table.insert(group, pdf.object.text({ text = "WORK" })
                :align_to(g.cell({ row = 1, col = 2 }), { v = "top", h = "left" }))
            table.insert(group, pdf.object.text({ text = "FAMILY / FRIENDS" })
                :align_to(g.cell({ row = 2, col = 1 }), { v = "top", h = "left" }))
            table.insert(group, pdf.object.text({ text = "RELATIONSHIP" })
                :align_to(g.cell({ row = 2, col = 2 }), { v = "top", h = "left" }))
        end,
    }))
    page.push(make_lined_section({
        bounds = grid.cell({ row = 24, col = 1, width = 7, height = 4 }),
        label = "I'M LOOKING FORWARD TO",
        lines = { "1", "2", "3" },
    }))
    page.push(make_lined_section({
        bounds = grid.cell({ row = 29, col = 1, width = 7, height = 4 }),
        label = "HABITS I'M FOCUSING ON DEVELOPING",
        lines = { "1", "2" },
    }))
    page.push(make_section({
        bounds = grid.cell({ row = 34, col = 1, width = 3.25, height = 3 }),
        label = "LEARN SOMETHING NEW",
    }))
    page.push(make_section({
        bounds = grid.cell({ row = 34, col = 4.75, width = 3.25, height = 3 }),
